...
export default async function Fruit({ params }: { params: { fruit: string } }) {
    ...
    return (
        ...
        <main className="w-11/12 m-auto">
            ...
            <Link href="/store" className="text-coal font-semibold text-md opacity-70">&lt; To all product</Link>
            ...
            <div className="flex justify-between">
                <div className={clsx("font-semibold px-4 py-1 border-2 rounded-lg border-coal border-opacity-50",
                    {
            ...
            <article className="my-8">
                ...
                <p className="text-coal mt-4 tracking-wide leading-7">{productInfo.detail}</p>
            </article>
            <div>
                <AccordionFruit nutrition={productInfo.nutrition} origin={productInfo.origin}/>
            </div>
        </main>
        <Order price={(productInfo.price * (1-productInfo.discount)).toFixed(2)} name={productInfo.name}/>
        <Footer/>
        </>
    )
}